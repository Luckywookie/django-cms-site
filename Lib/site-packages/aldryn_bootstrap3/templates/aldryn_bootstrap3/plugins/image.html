    ...
    {% if request.toolbar and request.toolbar.show_toolbar and request.toolbar.edit_mode %}
        {% if has_dnd_support %}
            <span class="js-filer-dropzone filer-dropzone filer-dropzone-image-plugin" data-filer-url="{% url 'admin:bootstrap3_image_ajax_upload' pk=instance.pk %}">
                ...
                {% endif %}
    {% endif %}
                <img data-target="#imagemodal" id="image-{{ instance.id }}"
                    ...
                    {% if instance.img_responsive or instance.shape or instance.thumbnail or instance.classes or request.toolbar %}
                        class="pop {% if request.toolbar and request.toolbar.show_toolbar and request.toolbar.edit_mode %}js-original-image {% endif %}{% if instance.img_responsive %}img-responsive{% endif %}{% if instance.shape %} img-{{ instance.shape }}{% endif %}{% if instance.thumbnail %} img-thumbnail{% endif %}{% if instance.classes %} {{ instance.classes }} {% endif %}"
                    {% endif %}
                    {% comment %}
...
<div class="modal fade" id="imagemodal{{ instance.id }}" tabindex="144" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" style="margin-top: 30px;">
  <div class="modal-dialog modal-lg">
    <div class="modal-content">
      <div class="modal-body">
        <button type="button" class="close" data-dismiss="modal" style="font-family: sans-serif;"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
        <img src="{{ instance.file.url }}" class="imagepreview" style="width: 100%;" >
          <p>{{ instance.id }}</p>
      </div>
    </div>
  </div>
...
<script>
    $(function() {
            $('#image-{{ instance.id }}').on('click', function() {
                $('.imagepreview').attr('src', $(this).find('img').attr('src'));
                $('#imagemodal{{ instance.id }}').modal('show');
            });
    });
</script>
